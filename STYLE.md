...
This file is only about coding style. See `AGENTS.md` for project architecture, commands, release workflow, and testing conventions.
...
As a general rule, we only use `!` for methods that have a correspondent counterpart without `!`. In particular, we do not use `!` to flag destructive actions. There are plenty of destructive methods in Ruby and Rails that do not end with `!`.

## Visibility modifiers

We do not add a newline under visibility modifiers, and we indent the content under them.
...
If a module only has private methods, we mark it `private` at the top and add an extra new line after but do not indent.
...
We model web endpoints as CRUD operations on resources (REST). When an action does not map cleanly to a standard CRUD verb, we introduce a new resource rather than adding custom actions.
...
In general, we favor a [vanilla Rails](https://dev.37signals.com/vanilla-rails-is-plenty/) approach with thin controllers directly invoking a rich domain model. We do not use services or other artifacts to connect the two.
...
When justified, it is fine to use services or form objects, but do not treat those as special artifacts:
